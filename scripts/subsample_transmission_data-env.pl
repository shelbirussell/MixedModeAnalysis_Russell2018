...
open SUBSAMPLEOUT, "> subsampledenvt_transmission_output.txt" or die "cannot open subsampledtaxa_transmission_output.txt\n" ;

foreach my$count (0..999) {
  my$subsampled_envt_taxa = subsample_envt(\%symbioses) ;
  my%subsampled_envt_taxa = %{$subsampled_envt_taxa} ;
  my($horizontaleq, $mixedeq, $verticaleq) = get_rates(\%subsampled_envt_taxa) ;
  #print $horizontaleq, "\t", $mixedeq, "\t", $verticaleq, "\n" ;
  my($hterrestrialeq, $mterrestrialeq, $vterrestrialeq, $haquaticeq, $maquaticeq, $vaquaticeq) = get_env(\%subsampled_envt_taxa) ;
  my($externalverticaleq, $internalverticaleq, $externalmixedeq, $internalmixedeq) = get_routes(\%subsampled_envt_taxa) ;
  my($horiznuteq, $mixnuteq, $vertnuteq, $horizdefeq, $mixdefeq, $vertdefeq, $horizmulteq, $mixmulteq, $vertmulteq, $horizunkeq, $mixunkeq, $vertunkeq, $horizmanipeq, $mixmanipeq, $vertmanipeq) = get_function(\%subsampled_envt_taxa) ;

  my@variables = ($horizontaleq, $mixedeq, $verticaleq) ;
  my$percmode = print_to_open_file (*SUBSAMPLEOUT, "perc_hmv_1evlveq".$count, \@variables) ;
  my@percmode = @{$percmode} ;
  push @Hp, $percmode[0] ;
  push @Mp, $percmode[1] ;
  push @Vp, $percmode[2] ;

  @variables = ($haquaticeq,$maquaticeq,$vaquaticeq) ;
  my$percaq = print_to_open_file (*SUBSAMPLEOUT, "aquaticeq".$count, \@variables) ;
  my@percaq = @{$percaq} ;
  @variables = ($hterrestrialeq,$mterrestrialeq,$vterrestrialeq) ;
  my$percter = print_to_open_file (*SUBSAMPLEOUT, "terrestrialeq".$count, \@variables) ;
  my@percter = @{$percter} ;
  push @HAp, $percaq[0] ;
  push @MAp, $percaq[1] ;
  push @VAp, $percaq[2] ;
  ...
  @variables = ($internalmixedeq,$externalmixedeq, $internalverticaleq, $externalverticaleq) ;
  my$percroute = print_to_open_file2 (*SUBSAMPLEOUT, "routeeq".$count, \@variables) ;
  my@percroute = @{$percroute} ;
  push @MIp, $percroute[0] ;
  push @VIp, $percroute[1] ;

  @variables = ($horiznuteq,$mixnuteq,$vertnuteq) ;
  my$percnut = print_to_open_file (*SUBSAMPLEOUT, "nutritioneq".$count, \@variables) ;
  my@percnut = @{$percnut} ;
  @variables = ($horizdefeq,$mixdefeq,$vertdefeq) ;
  my$percdef = print_to_open_file (*SUBSAMPLEOUT, "defenseeq".$count, \@variables) ;
  my@percdef = @{$percdef} ;
  @variables = ($horizmulteq,$mixmulteq,$vertmulteq) ;
  my$percmult = print_to_open_file (*SUBSAMPLEOUT, "multicomponenteq".$count, \@variables) ;
  my@percmult = @{$percmult} ;
  @variables = ($horizunkeq,$mixunkeq,$vertunkeq) ;
  my$percunk = print_to_open_file (*SUBSAMPLEOUT, "unknowneq".$count, \@variables) ;
  my@percunk = @{$percunk} ;
  @variables = ($horizmanipeq,$mixmanipeq,$vertmanipeq) ;
  my$percmanip = print_to_open_file (*SUBSAMPLEOUT, "manipulativeeq".$count, \@variables) ;
...
sub subsample_envt {
  ...
  my@shuffled_indices = shuffle(0..$#{$data{6}}) ;

  foreach my$i (@shuffled_indices) {
    if ($data{6}[$i] == 0 || $data{6}[$i] == 2) {
      if ($Acount < $min_count) {
        foreach my$column (0..8) {push @{$new_dataset{$column}}, $data{$column}[$i] ;}
        $Acount ++ ;
      }
    }
    if ($data{6}[$i] == 1) {
