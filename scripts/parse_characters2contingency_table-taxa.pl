...
while (<STDIN>) {
  ...
  push @{$data{$count}}, $split[2], $split[3], $split[4], $split[5]  ;

  ##host class
  $variables{"host_class:".$split[2]} = 1 ;
  ##host family
  $variables{"host_family:".$split[3]} = 1 ;
  ##symbiont phylum
  $variables{"sym_phylum:".$split[4]} = 1 ;
  ## symbiont family
  $variables{"sym_family:".$split[5]} = 1 ;
  if ($split[6] == 0) { $variables{"horizontal"} = 1 ; push @{$data{$count}}, "horizontal" ; }
  if ($split[6] == 1) { $variables{"mixed"} = 1 ; push @{$data{$count}}, "mixed" ; }
  if ($split[6] == 2) { $variables{"vertical"} = 1 ; push @{$data{$count}}, "vertical"; }

  if ($split[7] == 0) { $variables{"na"} = 1 ; push @{$data{$count}}, "na" ; }
  if ($split[7] == 1) { $variables{"external"} = 1 ; push @{$data{$count}}, "external" ; }
  if ($split[7] == 2) { $variables{"internal"} = 1 ; push @{$data{$count}}, "internal" ; }

  if ($split[8] == 0 || $split[8] == 2) { $variables{"aquatic"} = 1 ; push @{$data{$count}}, "aquatic" ; }
  if ($split[8] == 1) { $variables{"terrestrial"} = 1 ; push @{$data{$count}}, "terrestrial" ; }

  if ($split[9] == 0) { $variables{"nutrition"} = 1 ; push @{$data{$count}}, "nutrition" ; }
  if ($split[9] == 1) { $variables{"defense"} = 1 ; push @{$data{$count}}, "defense" ; }
  if ($split[9] == 2) { $variables{"multi_component"} = 1 ; push @{$data{$count}}, "multi_component" ; }
  if ($split[9] == 3) { $variables{"unknown"} = 1 ; push @{$data{$count}}, "unknown" ; }
  if ($split[9] == 4) { $variables{"manipulation"} = 1 ; push @{$data{$count}}, "manipulation" ; }
}
